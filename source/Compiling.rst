...
Parser wo not know what the source file mean for, instead, it just know
the token generated by the lexer, and the token object would use function
``next()`` to give token to the parser one by one.
